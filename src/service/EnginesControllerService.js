...
exports.createEngine = function(body) {
  return new Promise(function(resolve, reject) {
    console.log(body);
    sql.query("INSERT INTO engine (name, horsepower, brand) values (?,?,?)", [body.name, body.horsepower, body.brand], function (err, res) {
      if (err) {
        console.log(err);
        reject(err);
      }else{
...
exports.updateEngine = function(body,id) {
  return new Promise(function(resolve, reject) {
    sql.query("UPDATE engine SET name = ?, horsepower = ?, brand = ? WHERE id = ?", [body.name, body.horsepower, body.brand, id], function (err, res) {
      if (err) {
        console.log(err);
        reject(err);
      } else {
